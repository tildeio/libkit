...
const libkitDebug = BroccoliDebug.buildDebugCallback(`libkit:$package`);
...
module.exports = function({ name, packageName, root }) {
  return function(d) {
    ...
    return libkitDebug(build({ name, packageName, packageRoot: root }), `$${packageName}:$merged`);
  }
}
...
function build({ name, packageName, packageRoot }) {
  ...
  function debug(node, name) {
    return debug(node, `$${packageName}:${name}`);
  }
  ...
  if (isProdBuild()) {
    ...
    let testTree = tests([mergedBuilds], name, projectRoot);
...
function tests(modules, name, projectRoot) {
  let trees = libkitDebug(merge(modules), `$${name}:$webpack:$before`);
  let tests = libkitDebug(webpackTests(trees, name), `$${name}:$webpack:$after`);

  let testHTML = funnel(path.resolve(projectRoot, "test"), {
    include: ["index.html", "index.testem.html"]
  });

  testHTML = libkitDebug(testHTML, `$${name}:$dev-build:$html`);
